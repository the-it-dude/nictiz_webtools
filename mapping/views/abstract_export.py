...
class ExportEclSourceData(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            return Response(
                f"Probleem bij selecteren project. Bestaat het ID {pk}? Error: {e}"
            )

        # Fetch tasks
        filtered_tasks = (
            MappingTask.objects.order_by("id")
        ...
        # Fetch all comments
        comments = (
            MappingComment.objects.filter(comment_task__project_id=project)
        ...
        # Fetch all QA hits
        qa_hits = (
            MappingTaskAudit.objects.filter(task__project_id=project)
            .prefetch_related("user", "task")
            .values(
                "task__id",
                "audit_type",
        ...
        tasks = [
            {
                ...
                "ecl_queries": list(
                    filter(lambda item: (item["task__id"] == task.id), ecl_parts)
                ),
                "comments": list(
                    filter(lambda item: (item["task__id"] == task.id), comments)
                ),
                "qa_hits": list(
                    filter(lambda item: (item["task__id"] == task.id), qa_hits)
                ),
            }
