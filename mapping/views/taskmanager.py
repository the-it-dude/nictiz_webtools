...
class Permission_MappingTasks(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        if "mapping | taskmanager" in request.user.groups.values_list(
            "name", flat=True
        ):
            return True
...
class ChangeMappingTasks(viewsets.ViewSet):
    ...
    def create(self, request):
        print(
            f"[status/MappingStatuses create] requested by {request.user} - data: {str(request.data)[:500]}"
        )
        ...
        if payload.get("tasks"):
            ...
            if payload.get("action").get("changeUser") and payload.get("value").get(
                "changeUser"
            ):
            ...
            if payload.get("action").get("changeStatus") and payload.get("value").get(
                "changeStatus"
            ):
                new_status = MappingTaskStatus.objects.get(
                    id=payload.get("value").get("changeStatus")
                )
            ...
            if payload.get("action").get("changeProject") and payload.get("value").get(
                "changeProject"
            ):
                new_project = MappingProject.objects.get(
                    id=payload.get("value").get("changeProject")
                )
            ...
            if payload.get("action").get("changeCategory") and payload.get("value").get(
                "changeCategory"
            ):
...
class MappingTasks(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        data = (
            MappingTask.objects.filter(project_id=project)
            .order_by("id")
            .select_related(
                "user",
                "source_component",
            ...
            tasks.append(
                {
                    ...
                    "status": {"id": task.status.id, "title": task.status.status_title},
                    ...
                    "component_actief": task.source_component.component_extra_dict.get(
                        "Actief", "?"
                    ),
                }
            )
