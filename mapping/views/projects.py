...
class Projects(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        projects = (
            ...
            .select_related("source_codesystem", "target_codesystem")
        )
        ...
        for project in projects:
            ...
            tasks = (
                MappingTask.objects.filter(user=current_user, project_id=project)
    ...
    def retrieve(self, request, pk=None):
        ...
        # TODO filter on which projects the user has access to
        current_user = User.objects.get(id=request.user.id)
        project = MappingProject.objects.get(
            active=True, id=pk, access__username=current_user
        )
        ...
        tasks_per_status = []
        status_list = (
            tasks.order_by("status_id")
            .distinct("status")
            .values_list("status__status_title", flat=True)
        )
            ...
            tasks_per_status.append(
                {
                    ...
                    "count_user": tasks.filter(
                        status__status_title=status, user=current_user
                    ).count(),
                }
            )
        ...
        project_data = {
            ...
            "open_tasks": tasks.exclude(status=project.status_complete)
            .exclude(status=project.status_rejected)
            .count(),
            "open_tasks_user": tasks.filter(user=current_user)
            .exclude(status=project.status_complete)
            .exclude(status=project.status_rejected)
            .count(),
