...
from mapping.models import MappingProject, MappingTask
...
class api_UpdateCodesystems_post(UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        return self.request.user.groups.filter(
            name="mapping | admin codesystems"
        ).exists()

    def post(self, request, **kwargs):
        print(
            f"[update_codesystems/api_UpdateCodesystems_post post] requested by {request.user}"
        )

        try:
            print(
                "Diagnosethesaurus",
                json.loads(request.POST.get("codesystem[diagnosethesaurus]")),
            )
            ...
            print(
                "G-Standaard Diagnoses",
                json.loads(request.POST.get("codesystem[gstandaardDiagnoses]")),
            )
            ...
            if json.loads(
                request.POST.get("codesystem[snomed]")
            ):  # codesystem 1 = snomed
    ...
    def get(self, request, **kwargs):
        print(
            f"[update_codesystems/api_UpdateCodesystems_post get] requested by {request.user}"
        )

        return render(
            request,
            "mapping/v2/import_codesystems.html",
            {
...
class vue_MappingIndex(UserPassesTestMixin, TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        project_list = (
            MappingProject.objects.filter(active=True)
            .filter(access__username=current_user)
            .order_by("id")
        )
        project_dict = []
            ...
            project_dict.append(
                {
                    ...
                    "num_open_tasks": tasks.exclude(status=project.status_complete)
                    .exclude(status=project.status_rejected)
                    .count(),
                    "num_open_tasks_user": tasks.filter(user=current_user)
                    .exclude(status=project.status_complete)
                    .exclude(status=project.status_rejected)
                    .count(),
                }
            )
