...
def ecl_vs_rules(taskid):
    ...
    if task.project_id.project_type == "4":
        ...
        # Check if there are any rules present in db, otherwise skip this qa
        rules = MappingRule.objects.filter(
            project_id=task.project_id,
            target_component=task.source_component,
        )
        if rules.count() == 0:
            logger.info("Skipping [ecl_vs_rules] - no rules present")
        else:
            ...
            logger.info(
                f"[ECL vs RULES] Task {task.id} checking if the present rules SHOULD be there"
            )
            rules = MappingRule.objects.select_related(
            ...
            for rule in rules:
                valid = valid_concept_ids.get(
                    str(rule.source_component.component_id), False
                )
                if valid:
                    if rule.source_component.component_id == valid.get(
                        "id"
                    ) and rule.mapcorrelation == valid.get("correlation"):
                        # logger.info(f"[ECL vs RULES] Task {task.id} rule en correlation correct")
                        True
                    else:
                        logger.info(
                            f"[ECL vs RULES] Task {task.id} rule en correlation incorrect - HIT"
                        )
                        obj, created = MappingTaskAudit.objects.get_or_create(
                            task=task,
                            audit_type="Mismatch ECL vs rules",
                    ...
                    logger.info(
                        f"[ECL vs RULES] Task {task.id} / Rule not present - HIT"
                    )
                    obj, created = MappingTaskAudit.objects.get_or_create(
                        task=task,
                        audit_type="Mismatch ECL vs rules",
