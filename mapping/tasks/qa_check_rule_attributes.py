...
def check_rule_attributes(taskid):
    ...
    specimen_exclusion_list = json.loads(
        MappingCodesystemComponent.objects.get(component_id="123038009").descendants
    )
    ...
    logger.info(
        "Auditing project: #{0} {1}".format(task.project_id.id, task.project_id.title)
    )
    ...
    rules = MappingRule.objects.filter(
        project_id=task.project_id, source_component=task.source_component
    ...
    for rule in rules:
        ...
        if not mapping_prio_per_group.get(rule.mapgroup, False):
            mapping_prio_per_group.update({rule.mapgroup: [rule.mappriority]})
        else:
    ...
    for target in mapping_targets:
        other_rules = MappingRule.objects.filter(target_component=target).order_by(
            "source_component__component_id"
        )
        if other_rules.count() > 0:
            other_tasks_same_target = []
            for other_rule in other_rules:
                other_tasks = MappingTask.objects.filter(
                    source_component=other_rule.source_component
                )
                for other_task in other_tasks:
                    other_tasks_same_target.append(
                        f"{other_task.source_component.component_id} - {other_task.source_component.component_title}"
                    )

            for other_rule in other_rules:
                # Separate rule for project 3 (NHG Diagn-LOINC/SNOMED)
                if (rule.project_id.id == 3) and (
                    other_rule.target_component.component_id in specimen_exclusion_list
                ):
                    # logger.info('Project 3 -> negeer <<specimen voor dubbele mappings')
                    True
                # Separate rule for project 3 (NHG Diagn-LOINC/SNOMED)
                elif (rule.project_id.id == 13) and (
                    other_rule.target_component.component_id in specimen_exclusion_list
                ):
                    # logger.info('Project 13 -> negeer <<specimen voor dubbele mappings')
                    True
                elif (rule.project_id.id == 4) and (
                    other_rule.target_component.component_id
                    in json.loads(
                        MappingCodesystemComponent.objects.get(
                    ...
                    True
                elif (rule.project_id.id == 7) and (
                    other_rule.target_component.component_id
                    in json.loads(
                        MappingCodesystemComponent.objects.get(
                            component_id="118718002"
                        ).descendants
                    )
                ):
                    ...
                    if (other_rule.source_component != task.source_component) and (
                        task.source_component.codesystem_id
                        == other_rule.source_component.codesystem_id
                    ):
                        obj, created = MappingTaskAudit.objects.get_or_create(
                            task=task,
                            audit_type="Target used in multiple tasks",
                            hit_reason="Meerdere taken {} gebruiken hetzelfde target: {} - {}".format(
                                other_tasks_same_target,
                                other_rule.target_component.component_id,
                                other_rule.target_component.component_title,
                            ),
                        )
