...
from mapping.models import MappingRule, MappingTask, MappingTaskAudit
        ...
        for rule in rules:
            ...
            if related_rules.count() > 1:
                ...
                _tasks = MappingTask.objects.filter(
                    project_id=task.project_id, source_component=rule.target_component
                )
                tasks_with_errors = []
                for _task in _tasks:
                ...
                for task_with_error in tasks_with_errors:
                    ...
                    obj, created = MappingTaskAudit.objects.get_or_create(
                        ...
                        # hit_reason=f"SNOMED code dubbel gebruikt [{rule.source_component.component_id} |{rule.source_component.component_title}|].\n Ook gebruikt bij: {rule_list_flat}",
                        hit_reason=f"{rule_list_flat} gebruikt SNOMED code [{rule.source_component.component_id} |{rule.source_component.component_title}|] dubbel.",
                    )

    else:
        logger.info(
            f"[ECL check_duplicate_rules] Task {task.id} is not an ECL-1 task. No check."
        )
