...
from mapping.models import MappingEclPartExclusion, MappingTask, MappingTaskAudit
...
def test_recursive_ecl_exclusion(taskid):
    ...
    task = MappingTask.objects.select_related("source_component", "project_id").get(
        id=taskid
    )
    try:
        ...
        for exclusion in exclusions.components:
            ...
            try:
                other_task = MappingTask.objects.select_related("source_component").get(
