...
def snomed_daily_build_active(taskid):
    ...
    for mapping in mappings:
        ...
        if (
            str(mapping.source_component.codesystem_id.codesystem_title).upper()
            == "SNOMED"
        ):
            ...
            if active == False:
                hit = True
        if (
            str(mapping.target_component.codesystem_id.codesystem_title).upper()
            == "SNOMED"
        ):
            # Look for the code in the daily build browser of SI
            result = snowstorm.getConceptById(id=mapping.target_component.component_id)
        ...
        if hit:
            ...
            obj, created = MappingTaskAudit.objects.get_or_create(
                ...
                hit_reason="Een van de gebruikte SNOMED concepten ["
                + str(mapping.target_component.component_id)
                + "] is inactief in de daily build van SNOMED International",
            )
            logger.info(str(obj))
